...
fn print_features() {
    println!("\n=== Features ===");
    println!("Logging: {}", if features::LOGGING { "enabled" } else { "disabled" });
    println!("Metrics: {}", if features::METRICS { "enabled" } else { "disabled" });
    println!("Authentication: {}", if features::AUTHENTICATION { "enabled" } else { "disabled" });
}
